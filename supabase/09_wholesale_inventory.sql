-- Wholesale Inventory Redesign Migrations

-- 1. Modify Products Table
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS brand VARCHAR;
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS hsn_code VARCHAR;
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS product_type VARCHAR DEFAULT 'single';
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS gst_percent DOUBLE PRECISION;
...
CREATE INDEX IF NOT EXISTS purchase_invoices_shop_idx ON public.purchase_invoices(shop_id);
...
CREATE INDEX IF NOT EXISTS purchase_items_invoice_idx ON public.purchase_items(purchase_invoice_id);
...
CREATE INDEX IF NOT EXISTS batches_shop_idx ON public.batches(shop_id);
