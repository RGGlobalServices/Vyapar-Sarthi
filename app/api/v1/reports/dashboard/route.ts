...
export const GET = handle(async (req) => {
  ...
  // Execute dashboard queries sequentially to prevent Supabase connection pool exhaustion
  ...
  const returnsByReason = await prisma.materialReturn.groupBy({
    by: ['reason'],
    where: { shopId: shop.id, date: { gte: startDate, lte: endDate } },
    _sum: { amount: true },
    _count: { id: true },
  ...
  const realizedProfitData = await prisma.$queryRaw<{ realized_sales_profit: number }[]>`
    SELECT SUM(
      CASE 
  ...
  const udharPaymentsData = await prisma.$queryRaw<{ total_udhar_paid: number }[]>`
  ...
  const udharGivenData = await prisma.$queryRaw<{ total_udhar_given: number }[]>`
  ...
  const fastProd = await prisma.$queryRaw<any[]>`
  ...
  const slowProd = await prisma.$queryRaw<any[]>`
  ...
  if (shop.subscriptionPlan === 'wholesale') {
    const inventoryValueResult = await prisma.$queryRaw<{ total_value: number }[]>`
    ...
    const recentFeeds = await prisma.$queryRaw`
