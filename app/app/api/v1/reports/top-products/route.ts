...
export const GET = handle(async (req) => {
  ...
  if (groupBy === 'revenue') {
    const res = await prisma.$queryRaw<any[]>`
      SELECT p.name, SUM(si.price_per_unit * si.quantity) as value
      FROM sale_items si
    ...
    const res = await prisma.$queryRaw<any[]>`
      SELECT COALESCE(p.category, 'Uncategorized') as name, SUM(si.price_per_unit * si.quantity) as value
      FROM sale_items si
      JOIN sales s ON si.sale_id = s.id
      JOIN products p ON si.product_id = p.id
      WHERE s.shop_id = ${shop.id}::uuid AND s.created_at >= ${startDate} AND s.created_at <= ${endDate}
    ...
    const res = await prisma.$queryRaw<any[]>`
      ...
      JOIN sales s ON si.sale_id = s.id
      JOIN products p ON si.product_id = p.id
      WHERE s.shop_id = ${shop.id}::uuid AND s.created_at >= ${startDate} AND s.created_at <= ${endDate} AND s.payment_type = 'UDHAR'
      GROUP BY p.id, p.name
      ORDER BY value DESC
    `;
    items = res.map(r => ({ name: r.name, value: Number(r.value || 0) })).slice(0, limit);
